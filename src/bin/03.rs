...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    let re_one_line = Regex::new(r"\n").unwrap();
    let re_replace = Regex::new(r"(don't\(\).+?(do\(\)|$))").unwrap();
    let re_main = Regex::new(r"mul\((?<num1>\d{1,3}),(?<num2>\d{1,3})\)").unwrap();

    // Remove New-Lines \n
    let one_line = re_one_line.replace_all(&input, "").into_owned();

    // Remove Blocks of Don't -> (Do or Line end)
    let result = re_replace.replace_all(&one_line, "").into_owned();

    let mut ret = 0;

    let _ : Vec<_>= re_main.captures_iter(&result).map(|m| {
        let num1: u32 = m["num1"].parse::<u32>().unwrap();
        let num2: u32 = m["num2"].parse::<u32>().unwrap();
        ret += num1 * num2;
        true
